            ...
            <div class="table-1">
              <div class="flex s_bg">
                <div class="s_bg_flex">
                  ...
                  <el-input placeholder="请输入关键字查询" v-model="KeyWord" clearable size="small" class="s_padding filterSelect">
                  </el-input>
                  <el-select v-model="sortType" size="small" placeholder="请选择排序方式" class="s_padding">
                    <el-option v-for="item in sortTypes" :key="item.Code" :label="item.label" :value="item.Code">
                    </el-option>
                  </el-select>
                  <el-select v-model="moneyType" size="small" placeholder="请选择资金方向" class="s_padding">
                    <el-option v-for="item in moneyTypes" :key="item.Code" :label="item.label" :value="item.Code">
                    </el-option>
                  </el-select>
                  <button class="btn btn-default btn-sm btc" type="submit" @click="getExceptionOrderList()">
                    查询
                  </button>
                </div>
                <div class="s_bg_flex_right">
                  <button type="submit" id="exportBtn" @click="exportExcel('tableHeader','tableBody','异常委托汇总','异常委托汇总.xls','exportBtn')" class="btn btn-primary" style="float:right;font-size:12px;color:#fff">
              ...
              <table class="table table-1-1" id="tableHeader">
                <thead>
                  <tr>
                    ...
                    <th style="width: 10%">当前名称</th>
                    <th style="width: 25%">层级关系</th>
                  </tr>
                </thead>
              </table>
            </div>
          ...
          <div class="panel panel-default table-2" v-loading="loading">
            <table class="table table-hover table-2-1 table__243" id="tableBody">
              <tbody>
                ...
                <tr v-for="(item,index) in ExceptionDelegateList" :key="index" style="position: relative;">
                  <td style="width:5%;">{{index+1}}</td>
                  <td style="width:10%">{{item.UserCode}}</td>
                  <td style="width:10%">{{item.UserName}}</td>
                  <td style="width:10%">{{item.AgentCode}}</td>
                  <td style="width:10%">{{item.AgentName}}</td>
                  <td style="width:5%">{{item.MoneyDirection}}</td>
                  <td style="width:10%">{{item.TransferMoney}}</td>
                  <td style="width:5%">{{item.CurrencyName}}</td>
                  <td style="width: 25%">{{item.IB == null ? "--" : (item.IB == "" ? "--" : item.IB)}}</td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  data() {
    return {
      ...
      moneyType: 0,
      moneyTypes: [{ Code: 0, label: "入金" }, { Code: 1, label: "出金" }]
    };
  },
  components: {
    pageination
  ...
  filters: {
    GetDelegateType(type) {
      return EnumConfig.GetDelegateType(type);
    },
  ...
  methods: {
    getExceptionOrderList(isSearch = true) {
      if (isSearch) {
        this.pager = {
          PageNo: 1,
          PageSize: 20,
      ...
      this.$loginapi.postWithPage(
        ...
        response => {
          ...
          if (response.Ret == 0) {
            ...
            this.ExceptionDelegateList = response.Data.List;
    ...
    pageFn(size, pageno) {
      ...
      this.getExceptionOrderList(false);
    }
  }
};
</script>
