            ...
            <table class="table table-hover table-2-1 table__243" id="tableBody">
              <tbody>
                ...
                <tr v-for="(item,index) in ExceptionDelegateList" :key="index" style="position: relative;">
                  ...
                  <td style="width: 6%">{{item.ActionType | GetDelegateType}}</td>
                  <td style="width: 6%">{{item.OrderState | GetDelegateOrderStateType}}</td>
                  <td style="width: 12%">{{item.ExDateTime}}</td>
                  <td style="width: 30%" @mouseover="ShowText(item.ErrorMsg,index)" @mouseout="Hiddens()">{{item.ErrorMsg}}</td>
...
export default {
  ...
  methods: {
    getExceptionOrderList(isSearch = true) {
      ...
      let url = "/report/QueryExceptionOrderList";
      let postData = {
      ...
      this.$loginapi.postWithPage(
        ...
        response => {
          ...
          if (response.Ret == 0) {
            ...
            this.ExceptionDelegateList = response.Data;
